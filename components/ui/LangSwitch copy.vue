...
const isCurrentLocaleVisible = computed(() => {
  return availableLocales.value.length > 0;
});
</script>
<template>
  <div>
    <button v-if="isCurrentLocaleVisible" @click="LangSwitch" class="bg-transparent b-hidden cursor-pointer" :aria-label="t('a11y.langSwitch')">
